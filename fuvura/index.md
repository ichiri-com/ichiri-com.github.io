...
0〜100 歳の資産推移を可視化する、iOS 向けの個人資産シミュレーターです。複数の口座と商品を組み合わせ、NISA・iDeCo を含むポートフォリオの長期推移を試算し、グラフと年表で確認できます。<small>（入力したデータは利用者の端末内にのみ保存されます）</small>
...
たとえば NISA（つみたて投資枠・成長投資枠）と iDeCo を使い分け、特定口座にも資産があるとします。この時点で口座は 3 件——それが無料プランの上限です。それぞれの口座に商品を追加すれば、5 件はすぐに埋まります。

プレミアムプランなら、保有するすべての口座と商品をそのまま入力してシミュレーションできます。さらにシナリオを複数作成して、「60 歳に退職する場合」「65 歳まで働く場合」「積立額を増やした場合」を 1 枚のグラフに重ねて比較することもできます。将来の選択肢を数字とグラフで確かめたいときに役立ちます。
